...
def init_csv():
    ...
    if not os.path.exists('comment_data.csv'):
        with open('comment_data.csv', 'w', encoding='utf8', newline='') as file:
...
def getJsonHtml(url, params, retries=5):
    ...
    headers = {
        ...
        # 使用最新有效的Cookie（与arcType_spider相同，已验证成功）
...
def writeToCsv(row):
    ...
    写入csv操作（追加）
    """
    with open('comment_data.csv', 'a', encoding='utf8', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(row)
...
def start():
    ...
    print(f"共找到 {len(articleList)} 条微博，开始爬取评论...")
    for article in articleList:
        # article[0]是微博id，article[1]是微博内容（用于显示）
        print(f'正在爬取微博ID：【{article[0]}】的评论数据')
        time.sleep(1)  # 每条微博之间间隔1秒，避免请求过快
        params = {
            'id': article[0],
            'is_show_bulletin': 2
        }
        ...
        if jsonHtml:
            ...
            print(f"获取微博ID【{article[0]}】的评论失败，已跳过")
    print("微博评论信息爬取结束")
